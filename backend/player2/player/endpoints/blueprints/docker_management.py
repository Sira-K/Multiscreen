...
@docker_bp.route("/get_group_docker_status", methods=["GET"])
def get_group_docker_status():
    """Get Docker status for all groups or a specific group"""
    try:
        # Get the app state
        state = get_state()
        ...
        for gid, group in groups_to_check.items():
            ...
            if not container_id:
                docker_status[gid] = {
                    "group_name": group_name,
                    "running": False,
                    "message": "No container ID available"
                }
                continue
                
            # Check if the container is running
            cmd = ["docker", "ps", "-q", "--filter", f"id={container_id}"]
            ...
            if not is_running:
                # Container is not running, clear the ID from state
                with state.groups_lock if hasattr(state, 'groups_lock') else threading.RLock():
                    state.groups[gid]["docker_container_id"] = None
                    state.groups[gid]["status"] = "inactive"
                
            docker_status[gid] = {
                "group_name": group_name,
                "running": is_running,
                "container_id": container_id if is_running else None,
        ...
        return jsonify({
            ...
            "running_containers": len([s for s in docker_status.values() if s["running"]])
        }), 200
...
@docker_bp.route("/start_docker", methods=["POST"])
def start_docker():
    """Legacy endpoint - starts Docker for the first group or creates a default group"""
    try:
        state = get_state()
        ...
        if not state.groups:
            ...
            logger.info(f"Created default group: {default_group_id}")
        
        # Use the first group
        first_group_id = list(state.groups.keys())[0]
        
        # Call the group-specific start function
        from flask import current_app
        with current_app.test_request_context(json={"group_id": first_group_id}):
            return start_group_docker()
            
    except Exception as e:
        logger.error(f"Error in legacy start_docker: {e}")
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500

@docker_bp.route("/stop_docker", methods=["POST"])
def stop_docker():
...
def docker_status():
    """Legacy endpoint - returns status for all groups"""
    return get_group_docker_status()